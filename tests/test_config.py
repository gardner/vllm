...
def test_gb10_vllm_config_rejects_logprobs_logits_runtime(
    monkeypatch,
    logprobs_mode,
):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    ...
    model_config = ModelConfig("facebook/opt-125m", logprobs_mode=logprobs_mode)

    with pytest.raises(
        ValueError,
        match="logprobs logits runtime.*GB10/SM12x",
    ):
        VllmConfig(model_config=model_config)


@pytest.mark.parametrize("logprobs_mode", ["raw_logprobs", "processed_logprobs"])
def test_gb10_vllm_config_allows_logprobs_logprob_runtime(
    monkeypatch,
    logprobs_mode,
):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
...
@pytest.mark.parametrize("logprobs_mode", ["raw_logits", "processed_logits"])
def test_vllm_config_allows_logprobs_logits_runtime_off_gb10(
    monkeypatch,
    logprobs_mode,
):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
...
def test_gb10_vllm_config_rejects_skip_tokenizer_init_runtime(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
    ...
    with pytest.raises(
        ValueError,
        match="skip tokenizer init runtime.*GB10/SM12x",
    ):
        VllmConfig(model_config=model_config)


def test_vllm_config_allows_skip_tokenizer_init_runtime_off_gb10(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
...
def test_gb10_vllm_config_rejects_attention_dtype_override_runtime(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
    ...
    model_config = ModelConfig(
        "facebook/opt-125m",
        override_attention_dtype="float16",
    )

    with pytest.raises(
        ValueError,
        match="attention dtype override runtime.*GB10/SM12x",
    ):
        VllmConfig(model_config=model_config)


def test_gb10_vllm_config_allows_default_attention_dtype_runtime(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
    ...
    assert config.model_config is model_config
    assert config.model_config.override_attention_dtype is None


def test_vllm_config_allows_attention_dtype_override_runtime_off_gb10(
    monkeypatch,
):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
...
def test_gb10_vllm_config_rejects_fp64_gumbel_sampling_runtime(monkeypatch):
    ...
    with pytest.raises(
        ValueError,
        match="FP64 Gumbel sampling runtime.*GB10/SM12x",
    ):
        VllmConfig(model_config=model_config)


def test_gb10_vllm_config_allows_default_gumbel_sampling_runtime(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
...
def test_vllm_config_allows_fp64_gumbel_sampling_runtime_off_gb10(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
...
def test_gb10_vllm_config_rejects_hf_overrides_runtime(
    monkeypatch,
    hf_overrides,
):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    ...
    with pytest.raises(
        ValueError,
        match="HF overrides runtime.*GB10/SM12x",
    ):
        VllmConfig(model_config=model_config)
...
def test_vllm_config_allows_hf_overrides_runtime_off_gb10(
    monkeypatch,
    hf_overrides,
):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
...
def test_gb10_vllm_config_rejects_transformers_model_impl_runtime(monkeypatch):
    ...
    model_config = ModelConfig("facebook/opt-125m", model_impl="transformers")

    with pytest.raises(
        ValueError,
        match="Transformers model implementation runtime.*GB10/SM12x",
    ):
        VllmConfig(model_config=model_config)


def test_vllm_config_allows_transformers_model_impl_runtime_off_gb10(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
...
def test_gb10_vllm_config_rejects_prompt_embeds_runtime(monkeypatch):
    ...
    model_config = ModelConfig("facebook/opt-125m", enable_prompt_embeds=True)

    with pytest.raises(
        ValueError,
        match="prompt embeds runtime.*GB10/SM12x",
    ):
        VllmConfig(model_config=model_config)


def test_vllm_config_allows_prompt_embeds_runtime_off_gb10(monkeypatch):
    monkeypatch.setattr(platforms.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        platforms.current_platform,
        "is_device_capability_family",
