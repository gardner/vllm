...
_GB10_HF_CONFIG_PATH_RUNTIME_MESSAGE = (
    "HF config path runtime is not supported on GB10/SM12x in this fork: "
    "--hf-config-path and direct model_config.hf_config_path decouple the "
    "Hugging Face model configuration source from the model path before native "
    "vLLM model and backend selection outside the validated native first-path "
...
_GB10_CASCADE_ATTENTION_RUNTIME_MESSAGE = (
    "cascade attention runtime is not supported on GB10/SM12x in this fork: "
    "model_config.disable_cascade_attn=False opts into cascade attention "
    "heuristics and split attention execution outside the validated native "
    "first-path NVFP4 serving release. Keep cascade attention disabled on GB10 "
...
_GB10_FP64_GUMBEL_SAMPLING_RUNTIME_MESSAGE = (
    ...
    "Gumbel noise in sampler and speculative decoding Triton kernels outside "
    "the validated native first-path NVFP4 serving release. Use default FP32 "
...
class VllmConfig:
    ...
    def __post_init__(self):
        ...
        if (
            self.model_config is not None
            and self.model_config.hf_config_path
            and _is_gb10_sm12x_cuda_platform()
        ):
        ...
        if (
            self.model_config is not None
            and self.model_config.use_fp64_gumbel
            and _is_gb10_sm12x_cuda_platform()
        ):
