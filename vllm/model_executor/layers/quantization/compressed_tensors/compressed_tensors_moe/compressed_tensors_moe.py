...
def _gb10_w8a8_mxfp8_moe_loading_unsupported_reason() -> str | None:
    if not _is_sm12x_device():
        return None
    return (
        "CompressedTensors W8A8 MXFP8 MoE loading is "
        "not supported on GB10/SM12x. The "
        "generic MXFP8 MoE backend selection can prove reachability, but "
        "it is not native GB10 W8A8 MXFP8 MoE correctness evidence. Use a "
        "native SM12x W8A8 MXFP8 MoE backend after correctness evidence "
        "exists, or keep this checkpoint format unselected."
    )


def _gb10_w8a8_int_moe_loading_unsupported_reason() -> str | None:
    if not _is_sm12x_device():
        return None
    return (
        "CompressedTensors W8A8 Int8 MoE loading is "
        "not supported on GB10/SM12x. The "
        "generic Int8 W8A8 MoE backend selection can prove reachability, but "
        "it is not native GB10 W8A8 Int8 MoE correctness evidence. Use a "
        "native SM12x W8A8 Int8 MoE backend after correctness evidence "
        "exists, or keep this checkpoint format unselected."
    )


def _gb10_w4a8_int_moe_loading_unsupported_reason() -> str | None:
    if not _is_sm12x_device():
        return None
    return (
        "CompressedTensors W4A8 Int8 MoE loading is "
        "not supported on GB10/SM12x. The "
        "CPU-only W4A8 Int8 MoE backend selection can prove reachability, but "
        "it is not native GB10 W4A8 Int8 MoE correctness evidence. Use a "
        "native SM12x W4A8 Int8 MoE backend after correctness evidence "
        "exists, or keep this checkpoint format unselected."
    )
...
class CompressedTensorsMoEMethod(FusedMoEMethodBase):
    ...
    def get_moe_method(
        ...
        if quant_config._is_mxfp8(weight_quant):
            if reason := _gb10_w8a8_mxfp8_moe_loading_unsupported_reason():
        ...
        if quant_config._is_wNa16_group_channel(weight_quant, input_quant):
            ...
            if reason := _gb10_w8a8_int_moe_loading_unsupported_reason():
                raise ValueError(reason)
